...
DDL ON DATABASE begin
  declare
  v_osuser         audit_ddl.osuser%type;
  v_program        audit_ddl.program%type;
  v_module         audit_ddl.module%type;
  v_terminal       audit_ddl.terminal%type;
  v_logon_time     audit_ddl.logon_time%type;
--  v_dataexec       audit_ddl.dataexec%type;
  v_MysessionID    NUMBER;
  SendorAddress    varchar2(30)  := 'SPO12_LF@pergidao.com.br';
...
--        from dual;
        ...
        insert into SYS.AUDIT_DDL(USERNAME,TERMINAL,PROGRAM,MODULE,LOGON_TIME,OSUSER,OBJECT_NAME,OBJECT_TYPE,OBJECT_OWNER,COMMAND,DATAEXEC)
        values (ora_login_user,v_terminal,v_program,v_module,v_logon_time,v_osuser,ora_dict_obj_name,ora_dict_obj_type,ora_dict_obj_owner,ora_sysevent,SYSDATE);
        ...
        if v_osuser in ('pimentel18','evv') or ora_login_user  is null then
           raise other_session;
        end if;
       ...
       ' ========================================'|| crlf ||
       ' Usuario Oracle.: ' || ora_login_user || crlf ||
       ...
       when send_mail then
            ...
            ('Atenc?o! Voce executou um procedimento n?o permitido. Enviando e_mail para a Diretoria da Perdig?o');
            close cs_Rec;
       when others then
            DBMS_OUTPUT.PUT_LINE
